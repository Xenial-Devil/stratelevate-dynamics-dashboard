...
import axios from 'axios';
  ...
  const [name, setAggregationName] = useState('');
  const [displayName, setDisplayName] = useState('');
  const [description, setDescription] = useState('');

  const createAggregationType = (options, aggregationTypeObj) => {
    return new Promise((resolve, reject) => {
      ...
      let url = 'http://localhost:8080/api/aggregationTypes';
      alert("ji")
      axios
        .post(url, aggregationTypeObj, options)
        .then((response) => {
          ...
          if (response.data.status=='200 OK'){
            alert('Aggregation Type created!');
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      createAggregationType(options, aggregationTypeData).then((response) => {
        ...
        <div class="py-8 px-4 mx-auto max-w-6xl">
          <form onSubmit={handleSubmit}>
            ...
            <div class="sm:col-span-2">
                <label for="aggregationName"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    Aggregation Short Name
               </label>
               <input type="text" name="name" id="name" value={name} onChange={(e) => setAggregationName(e.target.value)}
                    ...
                    placeholder="Aggregation Short Name..." required="" />
            </div>
            <div class="sm:col-span-2">
                ...
                <input type="text" name="displayName" id="displayName" value={displayName} onChange={(e) => setDisplayName(e.target.value)}
                    class="bg-gray-100 border-2 border-2-gray-600 text-gray-900 placeholder-gray-600 text-sm rounded-lg focus:ring-primary-600 focus:border-2-primary-600 block w-full p-2.5 dark:bg-[#292929] dark:border-2-gray-600 dark:placeholder-gray-200 dark:text-white dark:focus:ring-primary-500 dark:focus:border-2-primary-500"
                    placeholder="Aggregation Name..." required="" />
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="description" rows="4" value={description} onChange={(e) => setDescription(e.target.value)}
                    class="block p-2.5 w-full text-sm text-gray-900 bg-gray-100 rounded-lg border-2 border-2-gray-600 placeholder-gray-600 focus:ring-primary-500 focus:border-2-primary-500 dark:bg-[#292929] dark:border-2-gray-600 dark:placeholder-gray-200 dark:text-white dark:focus:ring-primary-500 dark:focus:border-2-primary-500 resize-none"
